...
def plotAll(x: List[List[SimulationStep]]):
    for l in x:
        plt.plot([k for k in range(len(l)-1)], [l[k+1].Pnl for k in range(len(l)-1)])
    plt.show()
    return "ok"

performQueryRLQuery("ws://localhost:8799", query, lambda x: print("yahoo: ", x.Steps[0][0], x.Steps[0][1], x.Steps[0][203], plotAll(x.Steps)))
